...
#
# 用法：
#   bash scripts/e2e/run_e2e.sh           # 跑全部场景
#   bash scripts/e2e/run_e2e.sh skill     # 仅跑名为 skill 的场景
#
# 退出码：0 全过；非 0 = 失败的场景数。
...
if [ -z "$ONLY" ] || [ "$ONLY" = "repl" ]; then
    ...
    elif expect "$ROOT/scripts/e2e/repl_smoke.exp" >/dev/null 2>&1; then
        ok "repl"
    else
        ng "repl" "all REPL smoke checks pass" "see: expect scripts/e2e/repl_smoke.exp"
    fi
fi
