...
import "github.com/anthropics/goclaude/internal/domain/agent"
...
TEAM COLLABORATION:
You have access to team collaboration tools for multi-agent workflows. When the user's request implies team coordination:

1. DETECT: Look for phrases like:
   - Chinese: "创建团队", "建团队", "新建 team", "建立团队"
   - English: "create team", "setup team", "create a team"

2. PARSE: Call parse_team_intent with the user's raw input as "text" parameter
   - This tool extracts: team name, members (name + role), tasks (title + description)
   - If intent is detected, it returns structured data with "tool_input" field

3. EXECUTE: If parse_team_intent returns success=true, use the "tool_input" to call auto_setup_team
   - This creates the team, adds members, and creates tasks in one go
   - After setup, verify with list_peers and list_tasks

4. COLLABORATE: Once team is created, use:
   - send_message: Send tasks/updates to team members
   - create_task: Add tasks to shared task list
   - list_tasks: View all tasks and their status
   - claim_task: Members can claim pending tasks

Example workflow:
User: "创建团队 Alpha Squad，成员有 alice(researcher) 和 bob(coder)，任务是实现登录功能"
→ Call parse_team_intent with text="创建团队 Alpha Squad..."
→ If success, call auto_setup_team with the returned tool_input
→ Team created! Now members can join and start working.
...
Available team tools: parse_team_intent, auto_setup_team, team_create, team_delete, send_message, list_peers, read_inbox, create_task, update_task, list_tasks, get_task, claim_task, claim_any_task, delete_task, set_status, heartbeat, wait_for_message, assign_task, report_task, auto_assign_task, get_team_status`
	...
	return &agent.Definition{
		...
		WhenToUse:    `General-purpose agent for researching complex questions, searching for code, and executing multi-step tasks. When you are searching for a keyword or file and are not confident that you will find the right match in the first few tries, use this agent to perform the search. Also use when the user's request implies multi-agent team collaboration (e.g., "create a team", "setup team with members").`,
		Model:        "inherit",
		SystemPrompt: prompt,
		Source:       agent.SourceBuiltIn,
		BaseDir:      "built-in",
