...
// 同一条消息会以**相同的 ID**写入所有收件人的 inbox（便于追踪）。
func (s *TeamService) Send(in SendInput) (*SendResult, error) {
	...
	if msg.ID == "" {
		msg.ID = team.NewTextMessage("", "", "").ID // 借用 stamp 生成 ID
	}
...
func (s *TeamService) ReadInbox(teamName, agentName string, drain bool) ([]team.Message, error) {
	...
	if f.FindMemberByName(agentName) == nil {
...
// ProcessLeaderInbox 扫描 leader 的未读消息并执行副作用：
//
		...
		case team.MessageTaskResult:
			if m.TaskID != "" {
				...
				case team.TaskWorking:
					status = team.SharedTaskWorking
				case team.TaskResolved:
					status = team.SharedTaskCompleted
				case team.TaskFailed:
					status = team.SharedTaskBlocked
				}
				_ = s.UpsertTask(teamName, team.SharedTask{
					ID:        m.TaskID,
					Status:    status,
