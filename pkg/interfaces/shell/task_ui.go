...
// ────────────────── Credits 估算 ──────────────────
//
// 终端环境没有真实计费接口，这里用"上下文规模 + 输出规模"做一个稳定、可解释的
// 估算：约定 1 Credit ≈ 1000 tokens，token 数按 ~4 字符/token 粗略折算。
// 该估算仅用于界面呈现，不参与任何业务决策。

// approxTokens 粗略估算一段文本的 token 数（约 4 字符 / token，至少为 0）。
...
// approxMessagesTokens 估算整个消息列表的上下文 token 规模。
func approxMessagesTokens(msgs []query.Message) int {
	total := 0
	for _, m := range msgs {
...
// estimateSessionTokens 基于当前上下文规模给出本轮会话的 token 预估区间。
//
// 低值：上下文 + 一次较短回复；高值：上下文翻倍 + 一次较长（含工具往返）回复。
func estimateSessionTokens(msgs []query.Message) (low, high int) {
	ctx := approxMessagesTokens(msgs)
	low = ctx + 300
	high = ctx*2 + 6000
	return low, high
}
