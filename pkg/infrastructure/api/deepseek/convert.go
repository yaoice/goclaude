...
// resolveModel 将上层模型名映射到 DeepSeek 实际模型名
//
// 兼容用户使用 claude-* 名称（默认回退到 deepseek-chat）
func resolveModel(name string) string {
	if name == "" {
		return ModelChat
	}
	switch name {
	case ModelChat, ModelReasoner, "deepseek-coder":
		return name
	}
	if strings.HasPrefix(name, "deepseek-") {
		return name
	}
	// 非 DeepSeek 模型名 -> 回退到默认聊天模型
	return ModelChat
}
...
func convertAssistantMessage(msg query.Message) ChatMessage {
	...
	for _, block := range msg.Content {
		...
		case query.ContentTypeThinking:
			// 领域 thinking 块 → DeepSeek reasoning_content（多块时拼接）
			if block.Thinking != "" {
	...
	// DeepSeek/OpenAI 协议要求 assistant.content 字段必须存在：
	// - 有文本时：填文本
	// - 仅有 tool_calls 时：必须显式填 ""（不能省略，否则 deepseek 报
...
func convertChoiceToMessage(id string, m ChatMessage) *query.Message {
	...
	// reasoning_content（deepseek-reasoner 推理链）→ thinking 块，排在 text 之前
...
// convertUsage 转换 Usage（含 DeepSeek 特有的缓存命中信息）
...
// mapFinishReason DeepSeek finish_reason -> 领域 StopReason
