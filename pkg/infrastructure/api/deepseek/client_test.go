...
func TestBuildRequest_ToolCallRoundtrip(t *testing.T) {
	...
	// 应有 4 条消息：user / assistant(含tool_calls) / tool
...
// 否则 deepseek 服务端会报 `messages[i]: missing field content`。
func TestBuildRequest_AssistantToolCallsOnly_HasContentField(t *testing.T) {
	...
	if asst.Content == nil {
		t.Fatalf("assistant.Content is nil; deepseek will reject with 'missing field content'")
	}
	if *asst.Content != "" {
...
func TestResolveModel(t *testing.T) {
	cases := map[string]string{
		"":                         ModelChat,
		"deepseek-chat":            ModelChat,
		"deepseek-reasoner":        ModelReasoner,
		"deepseek-coder":           "deepseek-coder",
		"claude-sonnet-4-20250514": ModelChat, // 非 deepseek 名称回退
		"deepseek-custom-v2":       "deepseek-custom-v2",
	}
...
func TestStream_TextEvents(t *testing.T) {
	chunks := []string{
		`{"id":"1","model":"deepseek-chat","choices":[{"index":0,"delta":{"role":"assistant","content":"Hello"}}]}`,
		`{"id":"1","model":"deepseek-chat","choices":[{"index":0,"delta":{"content":" world"}}]}`,
		`{"id":"1","model":"deepseek-chat","choices":[{"index":0,"delta":{},"finish_reason":"stop"}],"usage":{"prompt_tokens":2,"completion_tokens":2,"total_tokens":4}}`,
	}
