...
// parseSSEStream 解析 OpenAI/Kimi 风格的 SSE 流，并转换为领域 StreamEvent
//
// 协议特点：
//   - 每个事件是一行 "data: {json}"
//   - 终止标记 "data: [DONE]"
//   - 推理链增量出现在 choices[0].delta.reasoning_content（Kimi K2 思考模型）
...
func (c *Client) parseSSEStream(ctx context.Context, body io.ReadCloser, events chan<- query.StreamEvent) {
	...
	for scanner.Scan() {
		...
		if err := json.Unmarshal([]byte(payload), &chunk); err != nil {
			c.logger.Warn("Kimi SSE 解析失败", "err", err, "payload", payload)
			continue
		}
...
// Kimi K2 流式顺序：reasoning_content → content → tool_calls
type streamState struct {
	nextBlockIdx int

	// 推理/思考块状态（Kimi K2 的 reasoning_content）
	...
	// 工具调用：Kimi index -> 我方 block index
...
func (s *streamState) handleDelta(ctx context.Context, events chan<- query.StreamEvent, d *Delta) bool {
	// 推理/思考增量（Kimi K2 的 reasoning_content）
