// Package kimi 实现 Kimi API 客户端，适配 query.AIProvider 接口
//
// Kimi 使用 OpenAI 兼容协议，本客户端在领域模型(query.*)与
// OpenAI 风格协议之间做双向转换，对外暴露与其他 Provider 相同的接口。
package kimi
...
const (
	// DefaultBaseURL Kimi API 默认地址（Moonshot 平台）
...
func (c *Client) Send(ctx context.Context, params *query.SendParams) (*query.Message, *query.Usage, error) {
	...
	if len(apiResp.Choices) == 0 {
		return nil, nil, fmt.Errorf("kimi: empty response choices")
	}
...
func (c *Client) doRequest(ctx context.Context, reqBody *ChatRequest) (*http.Response, error) {
	...
	for attempt := 0; attempt <= c.config.MaxRetries; attempt++ {
		if attempt > 0 {
			...
			c.logger.Debug("Kimi 重试请求", "attempt", attempt, "delay", delay)
			select {
			case <-ctx.Done():
				return nil, ctx.Err()
		...
		if resp.StatusCode == http.StatusTooManyRequests || resp.StatusCode >= 500 {
			...
			resp.Body.Close()
			lastErr = fmt.Errorf("kimi: status %d: %s", resp.StatusCode, string(b))
			continue
		}
	...
	return nil, fmt.Errorf("kimi: max retries exceeded: %w", lastErr)
}
...
func parseErrorResponse(status int, body []byte) error {
	...
	if json.Unmarshal(body, &er) == nil && er.Error.Message != "" {
		return fmt.Errorf("kimi API error (status %d, type=%s, code=%s): %s",
			status, er.Error.Type, er.Error.Code, er.Error.Message)
	}
	return fmt.Errorf("kimi API error (status %d): %s", status, string(body))
}
