// Package kimi 实现 Kimi Chat Completions API 客户端
// Kimi 使用与 OpenAI 兼容的协议（api.moonshot.cn）
package kimi

// ChatRequest Kimi Chat Completions 请求体（OpenAI 兼容）
...
// OpenAI 兼容协议在 assistant 消息含 tool_calls 时仍要求 content 字段必须存在
...
type ChatMessage struct {
	...
	ReasoningContent string     `json:"reasoning_content,omitempty"` // 推理链（Kimi K2 思考模型）
...
type Delta struct {
	...
	ReasoningContent string     `json:"reasoning_content,omitempty"` // Kimi K2 推理链增量
	ToolCalls        []ToolCall `json:"tool_calls,omitempty"`
}
